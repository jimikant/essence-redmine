...
module PortfoliosHelper
  ...
  def portfolio_project_status_select(selected)
  ...
  def filter_options_for_select(query)
    ungrouped = []
    grouped = {}
    query.available_filters.map do |field, field_options|
      if field_options[:type] == :date_past || field_options[:type] == :date
        group = %w(estimated_hours spent_time).include?(field)
      end
      if group
